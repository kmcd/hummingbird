...
class Gateway extends IbGateway {
  ...
  public void historicalData(int reqId, String date, double open, double high, double low, double close, int volume, int count, double WAP, boolean hasGaps) {
    ...
    if (!data[time_stamp])
      data[time_stamp] = [:]
    System.out.println("1.parsedDate = " + parsedDate);
    parsedDate['MINUTE'] -= 1
    System.out.println("2.parsedDate = " + parsedDate);

    def prev = data[parsedDate.format('yyyy-MM-dd HH:mm:ss:S')]
    data[time_stamp][stocks[reqId].symbol()] = prev ?: ((prev[stocks[reqId].symbol()] - close) / close)

//    println "[DATA] - ${reqId} - date:${time_stamp} close:${close} volume:${volume}"
    println data
  }
