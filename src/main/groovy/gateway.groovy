...
class Gateway extends IbGateway {
  ...
  def disconnect() {
    if (client_socket.isConnected()) client_socket.eDisconnect()
  }
  ...
  public void orderStatus(int orderId, String status, int filled, int
    ...
    // TODO: use a log message instead
    println("""Gateway.orderStatus - orderId:$orderId, status:$status, 
  ...
  public void historicalData(int reqId, String date, double open, 
    ...
    if (!data[time_stamp]) data[time_stamp] = [:]
    ...
    if (prevData == null) { stockReturn = 0 }
    else { stockReturn = (open - close) / close }
  ...
  def real_time_quotes(stock) {
    client_socket.reqMktData(Quote.request_id(stock.symbol()), 
      stock.contract, '', false)
  }
  ...
  def place_order(contract, order) {
    ...
    log.info("[ORDER] type:${order.m_action} symbol:${contract.m_symbol}
      quantity:${order.m_totalQuantity} price:${order.m_lmtPrice}")
    next_order_id
  }
  ...
  def eod_data(stock) {
    ...
    client_socket.reqHistoricalData(1, contract, '20120818 15:16:55 GMT', 
      '3 D', '1 day', 'TRADES', 1, 1)
  }
  ...
  void realtimeBar(int reqId, long time, double open, double high, double
    low, double close, long volume, double wap,
                   int count) {
    lastTime[reqId] = time
    def symbol = stocks[reqId].symbol()
  ...
  void updateAccountValue(String key, String value, String currency, String
    accountName) {
    if (key.equals('CashBalance')) IBUtils.setBalance(value as double)
  }

  void error(Exception e) {
    print "[ERROR] "
    e.printStackTrace()
    log.severe "${e.getMessage()} ${e.stackTrace}"
  }
  ...
  void execDetails(int orderId, Contract contract, Execution execution) {
    log.info("Order execution details: orderId:$orderId,
      orderId:${execution.m_orderId}, orderRef:${execution.m_orderRef},
      price:${execution.m_price}, shares:${execution.m_shares}, 
